...
def load_data_file_gift(file, stats):
	...
	training_df = pd.read_csv(
		file,
		skiprows = [0],
		names = ["broadcaster", "viewer", "product_name", "order_time", "count"], dtype = {
	...
	training_ds = (
		tf.data.Dataset.from_tensor_slices(
			({
				...
				"broadcaster": tf.cast(
					df['broadcaster'].values,
					tf.string
				),
				"product_name": tf.cast(
					df['product_name'].values,
					tf.string
				),
				"order_time": tf.cast(
					df['order_time'].values,
					tf.string
				),
				"count": tf.cast(
					df['count'].values,
...
def main(argv):
	...
	# Fetch the data
	dataset, nrow = load_training_gift("csv/f880878d-4944-40f8-b80f-f47b5841398c.csv", "")
