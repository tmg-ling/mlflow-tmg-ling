        ...
        self.viewer_embedding = tf.keras.Sequential(
            [
                ...
                tf.keras.layers.Embedding(
                    len(user_conf["unique_user_ids"]) + 1, user_conf["viewer_embedding_dimension"]
                    ),
            ]
        )
...
class TwoTowers(tf.keras.Model):
    ...
    def __init__(self, two_tower_broadcaster_model, two_tower_user_model, two_tower_task):
        super().__init__()
    ...
    def train_step(self, features: Dict[Text, tf.Tensor]) -> tf.Tensor:
        ...
        with tf.GradientTape() as tape:
            ...
            broadcaster_embeddings = self.broadcaster_model(
...
def load_data_file_gift(file):
    ...
    training_df = pd.read_csv(
        file,
        skiprows = [0],
        names = ["broadcaster", "viewer", "count"], dtype = {
    ...
    def __init__(self, loss, ranking_conf):
        ...
        self.user_embeddings = tf.keras.Sequential(
            [
                ...
                tf.keras.layers.Embedding(len(ranking_conf["unique_user_ids"]) + 1, embedding_dimension)
            ]
        )
        ...
        self.broadcaster_embeddings = tf.keras.Sequential(
            [
                ...
                tf.keras.layers.Embedding(len(ranking_conf["unique_broadcasters"]) + 1, embedding_dimension)
            ]
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--broadcaster_embedding_dimension", default = 96, type = int, help = "broadcaster_embedding_dimension"
    )
    parser.add_argument("--viewer_embedding_dimension", default = 96, type = int, help = "viewer_embedding_dimension")
...
def main():
    ...
    # Fetch the data
    local_file = "../csv/data_latest.csv"
    if not os.path.exists(local_file) and not os.path.isfile(local_file):
        filename = (
            "s3://tmg-machine-learning-models-dev/for-you-payer-training-data/data_latest.csv"
        )
    else:
        filename = local_file
    ...
    retrieval_results = {"viewer": [], "broadcaster": [], "retrieval_score": [], "count": []}
    for user_id in unique_user_ids:
        # for user_id in ['zoosk:de347500a97c284a84c1b14071f4c0cd', 'agged:5404088037']:
        scores, topk_broadcasters = retrieval_model({"viewer": tf.constant([user_id]), }, )
        topk_broadcasters = topk_broadcasters.numpy()[0][:100]
        scores = scores.numpy()[0][:100]
        retrieval_results["viewer"].append(user_id)
        d = dict(zip(df.loc[df["viewer"] == user_id, "broadcaster"], df.loc[df["viewer"] == user_id, "count"]))
        user_score = []
        user_broadcaster = []
        user_count = []
    ...
    with mlflow.start_run(run_name = "Gift Model Experiments Using Two Tower Model") as run:
        ...
        mlflow.log_metric("root_mean_squared_error", hist.history["root_mean_squared_error"][-1])
